...
```
## Compare median absolute number per time period between states
...
n_tweets_per_period = as.data.frame(df_sui %>% 
  group_by(state, keywords, period) %>%
  summarize(keywords_n = sum(keywords_n),
            total_n = sum(tot_n))%>% 
  ungroup()) %>% 
  arrange(keywords, period, state) %>% 
...
OvsW = n_tweets_per_period %>% 
  ...
  summarize(
    proptest = list(
      prop.test(x = c(keywords_n_Oregon, keywords_n_Washington), n = c(total_n_Oregon, total_n_Washington), alternative = "two.sided", correct = FALSE) 
...
OvsW$prop_Oregon = round(sapply(OvsW$proptest, function(x) x$estimate[1]), 3)
OvsW$prop_Washington = round(sapply(OvsW$proptest, function(x) x$estimate[2]), 3)
...
OvsW$ci_lower = round(sapply(OvsW$proptest, function(x) x$conf.int[1]), 3)
OvsW$ci_upper = round(sapply(OvsW$proptest, function(x) x$conf.int[2]), 3)
```

### Sum of tweets per time period and state
...
### Proportion test between states per time period
...
# Statistical test to compare levels per time period between Oregon and Washington

```{r}
#group by: period and query

# distribution had outlier peaks: use ranked sum tests
...
Or_vs_Wa = df_sui %>% 
  ...
  summarise(
    ttest = list(
       t.test(pr ~ state, var.equal = TRUE)
    ))  #this is a new alternative way for writing

Campaign.query_baseline = Or_vs_Wa$ttest[[1]]; Campaign.query_baseline
Campaign.query_pre.campaign = Or_vs_Wa$ttest[[2]]; Campaign.query_pre.campaign
Campaign.query_campaign.week = Or_vs_Wa$ttest[[3]]; Campaign.query_campaign.week
Campaign.query_after.campaign = Or_vs_Wa$ttest[[4]]; Campaign.query_after.campaign

#does not give the same df as: 
t.test(pr ~ state, data = filter(df_sui, period == "Baseline"& keywords =="Breaking the silence"), var.equal = T, paired = FALSE)
```
## Breaking the silence query

Comparing the percentage in Oregon vs. Washington 

* In the baseline period: `r apa_print(Campaign.query_baseline)$full_result`
* 2019 before campaign: `r apa_print(Campaign.query_pre.campaign)$full_result`
