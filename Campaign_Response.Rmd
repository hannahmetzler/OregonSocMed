...
OvsW$percent_difference= round(sapply(OvsW$proptest, function(x) x$estimate[1])-(sapply(OvsW$proptest, function(x) x$estimate[2])), 3)*100
...
OvsW$ci_lower = round(sapply(OvsW$proptest, function(x) x$conf.int[1]), 3)*100
OvsW$ci_upper = round(sapply(OvsW$proptest, function(x) x$conf.int[2]), 3)*100

pre_Breaking = filter(OvsW, period =="2019 before campaign" & keywords == "Breaking the silence")
```
...
### Should time periods be normalized for their length, because number of days influences the p-value?

Count of days per period
...
<!--   ungroup() %>%  -->
<!--   #campaign week -->
<!--   filter(date > as.Date('2019-04-05')) -->
<!-- # the campaign started and ended on a Sunday, these are 8 days, that should all be assigned to week 15 - the week after will have a day less? -->
